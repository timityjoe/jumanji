...
from jumanji.environments.routing.tarware.types import State
...
def test_tarware__specs(tarware_env: RobotWarehouse) -> None:
    """Validate environment specs conform to the expected shapes and values"""
    action_spec = tarware_env.action_spec
    observation_spec = tarware_env.observation_spec

    assert observation_spec.agents_view.shape == (2, 66)  # type: ignore
    assert action_spec.num_values.shape[0] == tarware_env.num_agents
    assert action_spec.num_values[0] == 5


def test_tarware__reset(tarware_env: RobotWarehouse) -> None:
    """Validate the jitted reset of the environment."""
    chex.clear_trace_counter()
    reset_fn = jax.jit(chex.assert_max_traces(tarware_env.reset, n=1))
    ...
    assert state1.grid.shape == (2, *tarware_env.grid_size)
...
def test_tarware__agent_observation(
    deterministic_tarware_env: Tuple[RobotWarehouse, State, TimeStep],
) -> None:
    """Validate the agent observation function."""
    env, state, timestep = deterministic_tarware_env
...
def test_tarware__step(tarware_env: RobotWarehouse) -> None:
    """Validate the jitted step function of the environment."""
    chex.clear_trace_counter()

    step_fn = chex.assert_max_traces(tarware_env.step, n=1)
    step_fn = jax.jit(step_fn)

    state_key, action_key1, action_key2 = random.split(random.PRNGKey(10), 3)
    state, timestep = tarware_env.reset(state_key)
    ...
    action1 = jnp.zeros((tarware_env.num_agents,), int).at[0].set(action1)
    action2 = jnp.zeros((tarware_env.num_agents,), int).at[0].set(action2)
...
def test_tarware__does_not_smoke(tarware_env: RobotWarehouse) -> None:
    """Validate that we can run an episode without any errors."""
    check_env_does_not_smoke(tarware_env)


def test_tarware__specs_does_not_smoke(
    tarware_env: RobotWarehouse,
) -> None:
    """Test that we can access specs without any errors."""
    check_env_specs_does_not_smoke(tarware_env)


def test_tarware__time_limit(tarware_env: RobotWarehouse) -> None:
    """Validate the terminal reward."""
    step_fn = jax.jit(tarware_env.step)
    state_key = random.PRNGKey(10)
    state, timestep = tarware_env.reset(state_key)
    assert timestep.first()

    for _ in range(tarware_env.time_limit - 1):
...
def test_tarware__truncation(
    deterministic_tarware_env: Tuple[RobotWarehouse, State, TimeStep],
) -> None:
    """Validate episode truncation based on set time limit."""
    tarware_env, state, timestep = deterministic_tarware_env
    step_fn = jax.jit(tarware_env.step)

    # truncation
    for _ in range(tarware_env.time_limit):
        state, timestep = step_fn(state, jnp.array([0, 0]))
...
def test_tarware__truncate_upon_collision(
    deterministic_tarware_env: Tuple[RobotWarehouse, State, TimeStep],
) -> None:
...
def test_tarware__reward_in_goal(
    deterministic_tarware_env: Tuple[RobotWarehouse, State, TimeStep],
) -> None:
